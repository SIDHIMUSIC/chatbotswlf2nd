...
async def paint(query, text, kb, entities=None):
    clean = _TG.sub("", text or "")
    if entities:
        try:
            await query.edit_message_caption(
                caption=clean, caption_entities=entities, reply_markup=kb
            )
            return True
        except Exception:
            pass
        try:
            await query.edit_message_text(
                clean, entities=entities, reply_markup=kb, disable_web_page_preview=True
            )
            return True
        except Exception:
            pass
